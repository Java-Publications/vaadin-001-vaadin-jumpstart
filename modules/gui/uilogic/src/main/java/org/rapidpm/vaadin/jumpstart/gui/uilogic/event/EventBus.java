...
public class EventBus {
    ...
    public static void register(Object objectWithHandler)
            ...
            if (handlerMethod.isAnnotationPresent(HandleEvent.class)) {
                ...
                if (parameterTypes.length != 1) {
                    throw new EventHandlerException(
                        "Method with annotation " + HandleEvent.class
                            .getSimpleName() + " can only have 1 parameter.");
                }
    ...
    private static void addHandlerToBus(Object objectWithHandler,
        Method handlerMethod, Class<? extends Serializable> eventClass) {
        HANDLER_MAP.compute(eventClass, (key, eventHandlerInvocations) -> {
            ...
            eventHandlerInvocations.add(
                new EventHandlerInvocation(objectWithHandler, handlerMethod));
    ...
    public static void unregister(Object objectWithHandler) {
        ...
        for (final Method handlerMethod : classWithHandler.getMethods()) {
            if (handlerMethod.isAnnotationPresent(HandleEvent.class)) {
                ...
                Class<? extends Serializable> eventClass = (Class<? extends Serializable>) parameterTypes[0];
                SortedSet<EventHandlerInvocation> eventHandlerInvocations = HANDLER_MAP
                    .get(eventClass);
                eventHandlerInvocations.stream().filter(
                    eventHandlerInvocation -> eventHandlerInvocation
                        .callsHandlerOf(objectWithHandler))
    ...
    public static void fireSynchronousEvent(Serializable event) {
        final SortedSet<EventHandlerInvocation> handlers = HANDLER_MAP
            .getOrDefault(event.getClass(), new ConcurrentSkipListSet<>());
        handlers.forEach(eventHandlerDelegator -> eventHandlerDelegator
            .invokeHandler(event));
    }
    ...
    public static void fireAsyncEvent(Serializable event) {
        final SortedSet<EventHandlerInvocation> handlers = HANDLER_MAP
            .getOrDefault(event.getClass(), new ConcurrentSkipListSet<>());
        handlers.forEach(eventHandlerDelegator -> EXECUTOR_SERVICE
            .submit(() -> eventHandlerDelegator.invokeHandler(event)));
    }
