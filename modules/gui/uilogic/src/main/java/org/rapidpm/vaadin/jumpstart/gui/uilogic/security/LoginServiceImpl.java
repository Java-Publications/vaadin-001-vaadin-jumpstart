...
public class LoginServiceImpl implements LoginService {
  ...
  private final BiFunction<String, String, User> mapToUser = (username, password) ->
      ("admin".equals(username) && "admin".equals(password)) ? new User(username, "Master", "Chief", of(2020, 1, 31)) :
          ("Sven".equals(username) && "123".equals(password)) ? new User(username, "Sven", "Ruppert", of(2020, 1, 31)) :
              User.NO_USER;


  private final BiFunction<String, String, Optional<User>> validate = (username, password) ->
      (Objects.isNull(username)) ? Optional.empty() :
          (Objects.isNull(password)) ? Optional.empty() :
