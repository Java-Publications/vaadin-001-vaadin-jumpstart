 ...
 */
public class BaseTestbenchTest extends BaseMicroserviceTest {
    ...
    private RemoteWebDriver getRemoteWebDriver() {
        ...
        case JAVAFX:
            ...
            remoteWebDriver = new JBrowserDriver(com.machinepublishers.jbrowserdriver.Settings.builder().requestHeaders(RequestHeaders.CHROME).userAgent(
                new UserAgent(UserAgent.Family.WEBKIT, "Google Inc.", "Win32", "Windows NT 6.1",
                    "5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2869.0 Safari/537.36",
                    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2869.0 Safari/537.36")).build());
            break;
        default:
            remoteWebDriver = new FirefoxDriver();
        }
    ...
    public String baseURL() {
        final String key = MainUndertow.SERVLET_PORT_PROPERTY;

        final String actualUsedServletPort = System.getProperty(key) == null
            ? MainUndertow.DEFAULT_SERVLET_PORT + ""
            : System.getProperty(key);

        return "http://localhost:" + actualUsedServletPort +"/" + MainUndertow.MYAPP;
    }
