package org.rapidpm.frp.core.model.matcher;

import java.util.function.Supplier;

import org.rapidpm.frp.core.model.Result;
import org.rapidpm.frp.core.model.Tuple;
...
public class Case<T> extends Tuple<Supplier<Boolean>, Supplier<Result<T>>> {
    ...
    public Case(final Supplier<Boolean> booleanSupplier,
    ...
    public static <T> Result<T> match(DefaultCase<T> defaultCase,
        ...
        for (final Case<T> matcher : matchers) {
            if (matcher.getT1().get())
                return matcher.getT2().get();
        }
