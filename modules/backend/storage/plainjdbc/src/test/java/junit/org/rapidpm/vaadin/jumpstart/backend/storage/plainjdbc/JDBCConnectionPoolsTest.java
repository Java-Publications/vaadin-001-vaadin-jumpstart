...
public class JDBCConnectionPoolsTest {
    ...
    public void testInitPool()
        ...
        final JDBCConnectionPools connectionPools = ((this.connectionPools
            == null) ? new JDBCConnectionPools() : this.connectionPools)
            .addJDBCConnectionPool(POOLNAME).withAutoCommit(false)
            .withJdbcURL("jdbc:hsqldb:mem:target/ivadb").withUsername("sa")
            .withPasswd("").withTimeout(2000).done();

        connectionPools.connectPools();

        final Connection connection = connectionPools.getDataSource(POOLNAME)
            .getConnection();
        try {
            connection.prepareStatement("drop table barcodes if exists;")
                .execute();
            connection.prepareStatement(
                "create table barcodes (id integer, barcode varchar(20) not null);")
                .execute();
            connection.prepareStatement(
                "insert into barcodes (id, barcode) values (1, '12345566');")
                .execute();

            // query from the db
            ResultSet rs = connection
                .prepareStatement("select id, barcode  from barcodes;")
                .executeQuery();
            rs.next();
            System.out.println(String
                .format("ID: %1d, Name: %1s", rs.getInt(1), rs.getString(2)));
    ...
    public void testGetNoneExistent() {
        HikariDataSource nope = ((this.connectionPools == null) ?
            new JDBCConnectionPools() :
            this.connectionPools).getDataSource("Nope");
